...
class ChooseRolesActivity : AppCompatActivity() {
    ...
    private fun distributePlayers()
    {
        ...
        while(numPlayers-- > 0)
        {
            ...
            when(RoleTypes.entries[roleIndex++])
            {
                ...
                RoleTypes.LECTER -> findViewById<NumberPicker>(R.id.numberPickerNumDrLecters).value++
            }
        }
        if(numPlayers > 0)
    ...
    private fun getListOfRoleNamesAndCount() : ArrayList<RoleTypeAndCount>
    {
        ...
        result.add(RoleTypeAndCount(RoleTypes.LECTER, findViewById<NumberPicker>(R.id.numberPickerNumDrLecters).value, true))
        return result
    }
}